...
$errors = array(); // Initialize an error array. #2
    ...
    mysqli_set_charset($conn,'UTF8');
     if(mysqli_query($conn,$sql)){
        ...
        $mail->Username = 'doanlinh1510@gmail.com';               // your SMTP username  
        $mail->Password = 'bomeyeumay6568';                      // your SMTP password  
        $mail->SMTPSecure = 'tls';                             // Enable TLS encryption, `ssl` also accepted  
        $mail->Port = 587;                                     // TCP port to connect to  
        $mail->setFrom('DoanHuuLinh1510.com', 'Linh Doan');  
        $mail->addAddress($email);                             // set your BCC email address  
        $mail->isHTML(true);                                   // Set email format to HTML  
        $mail->Subject = 'Activate the account.';  
        $mail->Body  = '<h1>Welcome to DoanHuuLinh.net</h1><h3>Dear '.$first_name.' '.$last_name.'</h3>';
        $mail->Body  .= '<p>Thank you for signing up to DoanHuuLinh.net. <br>We just need you to confirm your email address and finish setting up a new  account we created just for you. <br>You can do it super-quickly by clicking follow link:</p>';
        $mail->Body  .= '<b style="color: blue; text-decoration:none;"> click here http://localhost/BTL1/controller/process-activate.php?userid='.$id['userid'].'&code='.$randstring.' </b>';  
